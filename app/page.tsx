...
export default function Home() {
  // Define the emotional states with corresponding labels and color classes
  const emotions = [
    { value: 0, label: "😔 Lowkey Sad", color: "from-blue-500/50" },
  ...
  const [showDialog, setShowDialog] = useState(false); // Add this state

  const [emotion, setEmotion] = useState(50); // Default emotion set to "Vibing"
  const [mounted, setMounted] = useState(false);


  useEffect(() => {
    setMounted(true); // Ensures components are mounted before updating state
  }, []);

  // Find the current emotion based on the selected emotion
  const currentEmotion =
    emotions.find((em) => Math.abs(emotion - em.value) < 15) || emotions[2];
    {const features = [
      {
        icon: HeartPulse, // Pulse icon indicating support
        title: "Always Here for You",
        description: "AI support available at any moment, whenever you need to talk.",
        color: "from-rose-500/20", // Soft pink color for a comforting feel
        delay: 0.2,
      },
      {
        icon: Lightbulb, // Lightbulb icon for ideas and insights
        title: "Personalized Insights",
        description: "Get emotional insights tailored to your unique situation.",
        color: "from-amber-500/20", // Amber color to symbolize clarity and understanding
        delay: 0.4,
      },
      {
        icon: Lock, // Lock icon for security and privacy
        title: "Private & Secure",
        description: "Your mental well-being is private and protected with end-to-end encryption.",
        color: "from-emerald-500/20", // Green for security and calm
        delay: 0.6,
      },
      {
        icon: MessageSquareHeart, // Heart within a message bubble
        title: "Therapeutic Conversations",
        description: "Engage in evidence-based therapeutic conversations designed for healing.",
        color: "from-blue-500/20", // Blue for trust and communication
        delay: 0.8,
      },
    ];
    ...
    <div className="flex flex-col min-h-screen overflow-hidden">
      ...
      <section className="relative min-h-[90vh] mt-20 flex flex-col items-center justify-center py-12 px-4">
        ...
        >
          ...
          <User className="w-4 h-4 animate-wave text-primary" />
          ...
          AI That Guides You Through Every Emotion
          </span>
          </div>
          ...
          Embrace a new era of emotional care. Our AI companion is here to guide you with empathy and insight, every step of the way.
          </p>
